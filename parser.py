...
class Parser:
    def __init__(self, tokens):
        ...
        self.temp_count = 0 # keeping counter 
    ...
    def factor(self):
        ...
        if token[0] == 'NUMBER':
            ...
            self.consume()
            return token[1], []
        elif token[0] == 'LPAREN':# lpaen refers to left parenthesis
            self.consume()
            expr, code = self.expression()
            if self.current_token() and self.current_token()[0] == 'RPAREN':# rpaen refers to right parenthesis
                self.consume()
            return expr, code
    ...
    def new_temp(self):
#geenrating a temporary varibale
        self.temp_count += 1
...
#Breaking of words or in simple word tokeniztion
...
#3 address code printed as asked in documentation
print("\nIntermediate Code:")
